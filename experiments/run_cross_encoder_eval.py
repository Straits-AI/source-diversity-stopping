"""
Cross-encoder stopping policy evaluation — N=500 HotpotQA bridge questions.

Compares three policies:
  1. pi_ensemble          — brute-force baseline (all substrates each step)
  2. pi_aea_heuristic     — workspace-statistics stopping (our prior best)
  3. pi_cross_encoder_stop — content-aware stopping via MS MARCO cross-encoder
...
# Set API key BEFORE importing answer_generator — that module reads the env var
# at import time (module-level constant), so it must be set first.
...
from experiments.aea.policies.cross_encoder_stopping import CrossEncoderStoppingPolicy
...
RESULTS_FILE = RESULTS_DIR / "cross_encoder_eval.json"

LLM_POLICIES = ["pi_ensemble", "pi_aea_heuristic", "pi_cross_encoder_stop"]
...
def make_policies() -> list:
    return [
        ...
        CrossEncoderStoppingPolicy(top_k=5, max_steps=8),
    ]
...
def print_e2e_table(policy_names, agg_stats, ci_stats):
    print("=== E2E Evaluation (N=500, gpt-oss-120b) ===\n")
    header = (
        f"| {'Policy':<26} "
        f"| {'EM':>10} "
        f"| {'F1':>10} "
...
def run_cross_encoder_assessment(
    n_examples: int = N_EXAMPLES,
    seed: int = SEED,
    results_path=RESULTS_FILE,
) -> dict:
    """Full cross-encoder stopping evaluation pipeline."""
    random.seed(seed)
    np.random.seed(seed)

    print("=" * 70)
    print(f"Cross-Encoder Stopping — N={n_examples}, seed={seed}")
    print("=" * 70)

    # Phase 1: Load data
    print("\n[Phase 1] Loading HotpotQA data...")
    # Include the ijson-repaired file as the first candidate path so we
    # don't have to re-download when the canonical URL returns truncated data.
    _LOCAL_PATHS = [
    ...
    test_pairs = [
        ("pi_cross_encoder_stop", "pi_ensemble",     "CrossEncoder vs Ensemble"),
        ("pi_cross_encoder_stop", "pi_aea_heuristic","CrossEncoder vs AEA Heuristic"),
