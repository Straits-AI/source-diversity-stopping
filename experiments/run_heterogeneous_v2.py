"""
Heterogeneous Benchmark v2 Runner — AEA framework evaluation.

Runs all five policies (SemanticOnly, LexicalOnly, EntityOnly, Ensemble,
AEAHeuristic) on the 100-question heterogeneous benchmark v2 and reports:

  * Benchmark validation results (bridge-entity isolation, lexical overlap)
...
or as a module::
    ...
    from experiments.run_heterogeneous_v2 import run_heterogeneous_v2
    results = run_heterogeneous_v2()
"""
...
RESULTS_FILE = RESULTS_DIR / "heterogeneous_v2.json"
...
_TASK_TYPE_LABELS = {
    ...
    "multi_hop_chain":      "Multi-Hop Chain (2-hop)",
...
def prepare_dataset(examples: list[dict]) -> list[dict]:
    """
    Convert heterogeneous benchmark v2 examples to EvaluationHarness schema.
    ...
    for ex in examples:
        dataset.append({
            ...
            "bridge_entity": ex.get("bridge_entity", ""),
        })
    return dataset


# ─────────────────────────────────────────────────────────────────────────────
# Substrate switching analysis
# ─────────────────────────────────────────────────────────────────────────────
...
def _print_overall_table(all_results: dict, n: int) -> None:
    print(f"\n{'='*70}")
    print(f"=== Heterogeneous Benchmark V2 Results (N={n}) ===")
    print(f"{'='*70}")
    print("\nOverall:")
...
def _print_per_type_tables(all_results: dict, dataset: list[dict]) -> None:
    print("By Task Type:\n")
...
def run_heterogeneous_v2(
    seed: int = SEED,
    results_path: Optional[Path] = RESULTS_FILE,
) -> dict:
    """
    Run all five policies on the heterogeneous benchmark v2 and report results.
    ...
    results_path : Path, optional
        Path to write JSON results.  Pass None to skip.
    ...
    # Run built-in validation before evaluation
    validation_report = bench.validate()
    ...
    for task_type in _TASK_TYPES:
        ...
        print(f"  {_TASK_TYPE_LABELS[task_type]:<35}: {n_t:3d}")
    print()
